...
DummyDrawer::DummyDrawer(const Transform& transform,
	VECTOR localPos, float localRot) : transformParent_(transform)
{
	...
	transform_.SetModel(
		ins.LoadModelDuplicate(ResourceManager::SRC::DUMMY_DRAWER));

}

DummyDrawer::~DummyDrawer(void)
{
}

void DummyDrawer::Init(void)
{
	// モデル制御の基本情報
...
void DummyDrawer::Update(void)
{
	...
	switch (state_)
	{
	case DummyDrawer::STATE::CLOSE:
		break;
	case DummyDrawer::STATE::CLOSE_MOVE:
		if (moveCnt_ > 0.0f)
		{
		...
		break;
	case DummyDrawer::STATE::OPEN:
		break;
	case DummyDrawer::STATE::OPEN_MOVE:
		if (moveCnt_ > 0.0f)
		{
...
void DummyDrawer::Release(void)
{
}

Transform& DummyDrawer::GetTransform(void)
{
	return transform_;
}

const VECTOR& DummyDrawer::GetPos(void) const
{
	return transform_.pos;
}

void DummyDrawer::SetPos(VECTOR pos)
{
	transform_.pos = pos;
}

void DummyDrawer::Move(void)
{
...
bool DummyDrawer::IsHitPlayer(VECTOR pPos, std::vector<int> otherModelIds)
{
	float disPow1 = 0.0f;
	float disPow2;
...
DummyDrawer::STATE DummyDrawer::GetState(void)
{
	return state_;
}

void DummyDrawer::ChangeState(STATE state)
{
	// 状態の更新
	state_ = state;
...
void DummyDrawer::SyncParent(Transform& transform, VECTOR addAxis)
{
	// 親の回転情報を取得
